// Google Apps Script for Doctor's Logbook - GET Solution
...
function doGet(e) {
  try {
    ...
    switch (action) {
      case 'addPatient':
        return addPatient(e.parameter);
      case 'getPatients':
        return getPatients();
      case 'getPatient':
        return getPatient(e.parameter.id);
      case 'updatePatient':
        return updatePatient(e.parameter);
      case 'deletePatient':
        return deletePatient(e.parameter.id);
      default:
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === params.id) {
        // Update the row
        sheet.getRange(i + 1, 3).setValue(params.name);
        sheet.getRange(i + 1, 4).setValue(parseInt(params.age));
        sheet.getRange(i + 1, 5).setValue(params.gender);
        sheet.getRange(i + 1, 6).setValue("'" + (params.phone || '')); // Add apostrophe prefix
        sheet.getRange(i + 1, 7).setValue(params.email || '');
        sheet.getRange(i + 1, 8).setValue(params.visitDate);
        sheet.getRange(i + 1, 9).setValue(params.chiefComplaint);
        sheet.getRange(i + 1, 10).setValue(params.diagnosis || '');
        sheet.getRange(i + 1, 11).setValue(params.treatment || '');
        sheet.getRange(i + 1, 12).setValue(params.notes || '');
